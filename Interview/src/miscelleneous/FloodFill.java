...
public class FloodFill {
	...
	public static void main(String[] args) {
		...
		// Setup the matrix.
		int[][] matrix = new int[][] { { 1, 1, 1, 1, 1, 1 }, { 1, 1, 2, 2, 1, 1 }, { 1, 1, 2, 2, 1, 1 },
				{ 2, 2, 2, 2, 2, 2 } };
		...
		// Approach 1 : Using STACK.
		{
			...
			while (!stack.isEmpty()) {
				...
				for (int i = 0; i < directions.length; i++) {
					...
					if (x < 0 || x >= matrix.length || y < 0 || y >= matrix[0].length || matrix[x][y] != 2
			...
			System.out.println("\nTotal time taken by STACK approach : ["
					+ (System.nanoTime() - stackApproachStart) / 1000 + "] us\n");
		}
